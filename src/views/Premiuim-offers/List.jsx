...
class OffersList extends Component {
        ...
        options: {
          ...
          customBodyRender: (val) => (
            <div className="d-flex nowrap">
              <Link
                title="View Details"
                to={`/admin/premium-offers/${val}?lang=${this.state.selectedLang}`}
              >
              ...
              <Link
                className="ml-2"
                title="Edit"
                to={`/admin/premium-offers/edit/${val}?lang=${this.state.selectedLang}`}
              >
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevState.selectedLang !== this.state.selectedLang) {
      LangAPI.get(`/offers?lang=${this.state.selectedLang}`).then(
        (response) => {
          let rows = response?.data?.data?.filter((x) => x?.is_premium === 1);
  ...
  handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this ?")) {
      LangAPI.delete(`/offers/${id}?lang=${this.state.selectedLang}`)
        .then((response) => {
          if (response.status === 200) {
        ...
        .then(() => {
          LangAPI.get(`/offers?lang=${this.state.selectedLang}`).then(
            (response) => {
              let rows = response?.data?.data?.filter(
                (x) => x?.is_premium === 1
              );
              this.setState({ rows });
            }
      ...
      <div>
        ...
        {isCategoryFormOpen && (
          <CategoryDialog
            ...
            handleClose={() => this.setState({ isCategoryFormOpen: false })}
