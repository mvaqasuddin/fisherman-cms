...
export default function OfferPage() {
  ...
  const handleImageSelect = (e, index, section) => {
    ...
    if (e.target.checked) {
      ...
      setGallery({
        ...gallery,
        [section]: { ...gallery[section], section_avatar: "" },
      });
      setThumbnailPreview("");
  ...
  const handleSubmit = () => {
    let updatedOffer = { ...gallery };
    updatedOffer.meta = { ...seoInfo };
    updatedOffer.page_id = pageId;
    updatedOffer.slug = "offer-sections";

    LangAPI.post(`/add-section?lang=${selectedLang}`, updatedOffer)
      .then((response) => {
        if (response.status === 200) {
    ...
    <div>
      <div className={classes.root}>
        <Card>
          ...
          >
            <h4 className="mb-0">Add Offers Page</h4>
            <FormControl
              variant="outlined"
              size="small"
