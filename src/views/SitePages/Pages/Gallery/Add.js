...
export default function AddGallery() {
  ...
  const handleImageSelect = (e, index, section) => {
    ...
    if (e.target.checked) {
      if (isSingle && !isImagesList) {
        setGallery({
          ...gallery,
          [section]: { ...gallery[section], section_avatar: imagesData[index] },
        });
        ...
        const index2 = gal?.imageGallery?.findIndex(
          (img) => img._id === imagesData[index]._id
        );
        if (index2 === -1) {
      ...
      let gal = { ...gallery };
      let updateData = gal.imageGallery.filter(
        (u) => u._id !== imagesData[index]._id
      );
  ...
  const handleSubmit = (id, name) => {
    API.post(`/add_section`, gallery[name])
      .then((response) => {
        if (response.status === 200) {
          alert("Section updated successfully !");
        }
    ...
    LangAPI.post(`/add-section?lang=${selectedLang}`, updatedGalery)
      .then((response) => {
        if (response.status === 200) {
          alert("Section updated successfully !");
        }
  ...
  const handleRemoveSelectedImage = (x, arrayListType) => {
    ...
    switch (arrayListType) {
      case "uploadsPreview":
        let updatePreview = gallery?.imageGallery?.filter(
          (u) => u._id !== x._id
        );
      ...
      case "selectedImages":
        ...
        break;
      default:
        return setUploadsPreview(
          gallery?.imageGallery?.filter((u) => u._id !== x._id)
        );
    }
  };
  ...
  return (
    <div>
      <div className={classes.root}>
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12} sm={12}>
                    ...
                    <div className="thumbnail-preview-wrapper-large img-thumbnail">
                      {!gallery.banner.id > 0 ? (
                        gallery.banner.section_avatar?.url !== "" ? (
                          <img
                            ...
                            alt={gallery.banner.section_avtar_alt || ""}
                          />
                        ) : (
                          <img
                      ...
                      ) : typeof gallery.banner.section_avatar?.url ===
                        typeof 0 ? (
                    ...
                    {gallery?.imageGallery?.length > 0 &&
                      imagesData
                        ?.filter(function (array_el) {
                          return (
                            gallery?.imageGallery?.filter(function (
                              menuItems_el
                            ) {
                        ...
                        ?.map((x) => (
                          <SelectedImagesThumbnails
                            ...
                            handleRemoveSelectedImage={(r) =>
                              handleRemoveSelectedImage(r, "selectedImages")
                            }
                          />
                        ))}
                    {/* {uploadsPreview &&
        ...
        <Grid item xs={12} sm={12}>
          <MaterialButton
            onClick={() => handleSubmit(gallery.banner.id, "banner")}
            size="large"
            color="primary"
            variant="contained"
          >
