...
export default withRouter(function AddRoom(props) {
  ...
  const handleImageSelect = (e, index) => {
    if (e.target.checked) {
      if (isSingle && !isBanner && !isAuthorImg) {
        setRoom({ ...room, img: imagesData[index].url });
        setThumbnailPreview(imagesData[index].url);
        setTimeout(() => {
          setShowGallery(false);
        }, 500);
      } else if (isBanner && !isSingle && !isAuthorImg) {
        setRoom({ ...room, banner_img: imagesData[index].url });
        setBannerThumbnailPreview(imagesData[index].url);
        setTimeout(() => {
          setShowGallery(false);
        }, 500);
      } else if (isAuthorImg && !isSingle && !isBanner) {
        setRoom({ ...room, author_img: imagesData[index].url });
        setAuthorThumbnailPreview(imagesData[index].url);
        setTimeout(() => {
          setShowGallery(false);
        }, 500);
      }
  ...
  const handleSubmit = () => {
    ...
    if (isEdit) {
      LangAPI.post(`/blogs?lang=${selectedLang}`, finalRoom).then(
        (response) => {
          if (response.status === 200) {
            alert("Blog Updated");
                ...
                <Grid item xs={12} sm={6}>
                  <div className="thumbnail-preview-wrapper img-thumbnail">
                    {!isEdit ? (
                      ...
                      <img
                        src={process.env.REACT_APP_IMAGE_BASE_URL + room.img}
                        alt={room.alt_text || ""}
                      />
                    )}
                  </div>
                ...
                <Grid item xs={12} sm={6}>
                  <div className="thumbnail-preview-wrapper img-thumbnail">
                    {!isEdit ? (
                      ...
                      <img
                        src={
                          process.env.REACT_APP_IMAGE_BASE_URL + room.banner_img
                        }
                        alt={room.alt_text || ""}
                      />
                    )}
